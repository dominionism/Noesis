...
```bash
INIT=$(node "$HOME/.agents/get-shit-done/bin/gsd-tools.cjs" state load)
...
```bash
node "$HOME/.agents/get-shit-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
